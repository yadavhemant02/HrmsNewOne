...
const INTERN_OFFER_HEADER = (organizationName) =>
  `Thank you for your interest in building a career with ${organizationName} (hereinafter referred to as the "Organization"). We are pleased to extend to you an offer for an internship with us.`;
...
const INTERN_OFFER_FOOTER = (
  organizationName
) => `Please acknowledge your acceptance of this internship by signing and returning a copy of this letter within five (5) working days.

We look forward to having you onboard and believe this internship will be an enriching experience.

Warm regards,`;

const CONSULTANT_OFFER_HEADER = (organizationName) =>
...
const PARTTIME_OFFER_HEADER = (organizationName) =>
  `We are pleased to offer you a part-time position at ${organizationName}.`;
...
const PARTTIME_OFFER_FOOTER = (
  organizationName
) => `Please confirm your acceptance of the terms by signing and returning a copy of this letter within five (5) working days.
...
const OfferletterEditDialog = ({
  ...
  const fetchOfferContent = async () => {
    try {
      ...
      if (response.data && Object.keys(response.data).length > 0) {
        ...
        console.log(apiPoints, "llllllllllllllllllll");
        ...
        setFormData({
          ...
          companyAddress:
            response.data.companyAddress || defaults.companyAddress,
        });
      ...
      // showSnackbar('Error fetching offer content', 'error');
      showSnackbar(
        "Info | This is the default content. Once you upload, it will extract your OfferLetter content.",
        "info"
      );
  ...
  const handleAddPoint = () => {
    if (newPoint.trim()) {
      console.log(formData.offerSummury, newPoint.trim());
      setIsUsingDefault(false);
      setFormData((prev) => ({
        ...prev,
        offerSummury: [...prev.offerSummury, newPoint.trim()],
      }));
      console.log(formData.offerSummury, "llllllllllllllllllllll");
      setNewPoint("");
    }
  };
  ...
  const handlePdfUpload = async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        // Update company address if available in header
        if (extractedData.header) {
          const addressMatch = extractedData.header.match(/([^\n]+\n[^\n]+\n[^\n]+)/);
          if (addressMatch && addressMatch[1]) {
            handleChange('companyAddress', addressMatch[1].trim());
          } else {
            handleChange('companyAddress', defaults.companyAddress);
          }
        } else {
          handleChange('companyAddress', defaults.companyAddress);
        }
                    ...
                    {formData.offerSummury.map((point, index) => (
                      ...
                      >
                        <TextField
                          ...
                          size="small"
                          sx={{
                            "& .MuiInputBase-input": { whiteSpace: "pre-line" },
                          }}
                        />
                        <ListItemSecondaryAction>
                          <IconButton
...
export default OfferletterEditDialog;
