...
const DEFAULT_ADDRESS_PROOF_POINTS = [
  "This is to certify that the employee is a bonafide employee of the organization and has been working with us.",
  "The employee's current residential address as per our records is as mentioned above.",
...
const DEFAULT_ADDRESS_PROOF_HEADER = (organizationName) => 
  `This is to certify that the employee is a bonafide employee of ${organizationName} and has been working with us.`;

const DEFAULT_ADDRESS_PROOF_FOOTER = (organizationName) => 
  `This letter is issued at the request of the employee for the purpose of address verification. For any further clarification or information, please feel free to contact the undersigned.`;

const DEFAULT_COMPANY_ADDRESS = "The Hive at VR Bengaluru, ITPL Main Road\nMahadevpura, Bengaluru - 560048";

const AddressProofEditDialog = ({ open, onClose, organizationCode, organizationName, onSave }) => {
  ...
  useEffect(() => {
    if (open && organizationCode) {
      fetchAddressProofContent();
    }
  }, [open, organizationCode, organizationName]);

  const fetchAddressProofContent = async () => {
    try {
      setLoading(true);
      const response = await axios.post(
        `${base_hr}/hr-handler/increment/find-increment-leter-content?organizationCode=${organizationCode}&content=addressproof`
      );
      ...
      if (response.data) {
        ...
        const points = apiPoints.length > 0 ? apiPoints : DEFAULT_ADDRESS_PROOF_POINTS;
        
        setFormData({
          header: response.data.result.header || DEFAULT_ADDRESS_PROOF_HEADER(organizationName),
          incrementPoints: points,
          footer: response.data.result.footer || DEFAULT_ADDRESS_PROOF_FOOTER(organizationName),
      ...
      console.error('Error fetching address proof content:', error);
      // showSnackbar('Error fetching address proof content', 'error');
      showSnackbar('Info | This is the default content. Once you upload, it will extract your addressProof content.', 'info');
  ...
  const handleSave = async () => {
    try {
      ...
      if (response.status === 200) {
        showSnackbar('Address proof letter content saved successfully', 'success');
      }
    } catch (error) {
      console.error('Error saving address proof content:', error);
      showSnackbar('Error saving address proof content', 'error');
    } finally {
      setLoading(false);
      onClose();
    }
      ...
      >
        <DialogTitle>
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Typography variant="h6">Edit Address Proof Letter Content</Typography>
            <Box>
              <Button
                variant="outlined"
                color="secondary"
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <TextField
                  ...
                  fullWidth
                  variant="outlined"
                  helperText="Enter the header content for the address proof letter"
                />
              </Grid>
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  fullWidth
                  variant="outlined"
                  helperText="Enter the footer content for the address proof letter"
                />
              </Grid>
...
export default AddressProofEditDialog; 