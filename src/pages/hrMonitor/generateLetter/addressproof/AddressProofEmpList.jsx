...
import ViewAddressProofDialog from './ViewAddressProofDialog';
...
class AddressProofService {
  ...
  static async getAllGeneratedAddressCodes() {
  ...
  const fetchLetters = useCallback(async () => {
    try {
      ...
      const [generatedCodes, employeeData] = await Promise.all([
        AddressProofService.getAllGeneratedAddressCodes(),
        AddressProofService.getAllEmployeeDetails()
      ]);

      // Filter existing employees who have generated address proof letters
      ...
      const errorMessage = err.response?.data?.message || 
                          err.message || 
                          'Failed to fetch address proof letters. Please try again later.';
      setError(errorMessage);
      console.error('Error fetching address proof letters:', err);
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <TableCell colSpan={TABLE_COLUMNS.length}>
      <EmptyStateContainer>
        <Typography variant="h6" color="text.secondary" gutterBottom>
          No Address Proof Letters Found
        </Typography>
        <Typography variant="body2" color="text.secondary">
          {searchTerm 
            ? "No matching address proof letters found. Try adjusting your search criteria." 
            : "There are currently no generated address proof letters in the system."}
        </Typography>
        {searchTerm && (
          <Button 
            sx={{ mt: 2 }}
  ...
  <StyledTableRow>
    ...
    <StyledTableCell align="center">
      <Box sx={{ display: 'flex', justifyContent: 'center' }}>
        <Tooltip title="View Address Proof Letter">
          <ActionButton 
            size="small" 
            color="primary"
            onClick={() => onView(letter)}
            aria-label="View address proof letter"
          >
            <VisibilityIcon fontSize="small" />
          </ActionButton>
        </Tooltip>
        <Tooltip title="Download Address Proof Letter">
          <ActionButton 
            size="small" 
            color="success"
            onClick={() => onDownload(letter)}
            disabled={downloadLoading}
            aria-label="Download address proof letter"
          >
            <DownloadIcon fontSize="small" />
          </ActionButton>
        </Tooltip>
        <Tooltip title="Regenerate Address Proof Letter">
          <ActionButton 
            size="small" 
            color="secondary"
            onClick={() => onRegenerate(letter)}
            aria-label="Regenerate address proof letter"
          >
...
const AddressProofEmpList = () => {
  const theme = useTheme();
  const navigate = useNavigate();
  
  // Custom hooks
  const { letters, loading, error, refreshLetters } = useAddressProofLetters();
  const { 
    page, 
    rowsPerPage, 
    handlePageChange, 
  ...
  const handleDownloadLetter = useCallback(async (letter) => {
    try {
      ...
      const response = await AddressProofService.downloadAddressProof(letter.empCode);
  ...
  const handleRegenerateLetter = useCallback((letter) => {
    const employeeData = {
      ...
      city: letter.city,
      state: letter.state,
      postalCode: letter.pinCode,
      
      // Bank Details
      bankName: letter.bankName,
      bankAccount: letter.accountNumber,
      branchName: letter.branchName,
      ifseCode: letter.ifseCode,
      
      // Government IDs
      panNumber: letter.panNumber,
      aadharNumber: letter.aadharNumber,
    ...
    navigate('/dashboard-hr/address-proof-letter', { 
      state: { 
        employeeData,
  ...
  const handleGenerateNew = useCallback(async () => {
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        const existingEmployees = response.data.filter(emp => emp.current === 'EXIST');
        
        if (existingEmployees.length > 0) {
          const employeeData = existingEmployees[0]; // Get first employee by default
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      <HeaderContainer>
        <Box>
          <Typography variant="h4" component="h1" fontWeight="bold" color="primary.main">
            Generated Address Proof Letters
          </Typography>
          <Typography variant="subtitle1" color="text.secondary" sx={{ mt: 0.5 }}>
            View and manage all generated address proof letters
          </Typography>
        </Box>

        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <SearchTextField
            placeholder="Search address proof letters..."
            value={searchTerm}
      ...
      <ControlsContainer>
        ...
        >
          Generate New Address Proof Letter
        </GenerateNewButton>
      </ControlsContainer>
      ...
      >
        <StyledTableContainer>
          <Table stickyHeader aria-label="address proof letters table">
            <TableHeader />
            <TableBody>
...
export default AddressProofEmpList;